...
const NewSellerWelcome = ({ accountStatus, onClose }) => {
  // Determine verification status based on accountStatus or assume a default
  const isVerified = accountStatus ? (accountStatus.detailsSubmitted && accountStatus.payoutsEnabled) : false;
  const isPending = accountStatus ? (accountStatus.detailsSubmitted && !accountStatus.payoutsEnabled) : false;
  
  // If no account status is provided, assume the user has signed up but needs to complete verification
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          ...
          {needsVerification && (
            <div className="flex items-center text-red-700">
              ...
              <span>Your account needs additional information. Please complete the seller onboarding process.</span>
            </div>
          )}
        </div>
