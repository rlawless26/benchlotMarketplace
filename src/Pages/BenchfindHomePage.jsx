 ...
 *
 * Layout: Hero (left = display headline + stats, right = DropZone) → "What
 * you get back" cards → "Lately on Benchfind" strip → footer.
 *
 * DropZone accepts photo upload OR eBay URL paste. Photo flow hits /toolscan
 * directly. URL flow navigates to /check (existing surface) which already
 * handles the canonical check pipeline.
 *
 * Result rendering is a minimal placeholder for v1 of this rebuild — the
 * full Benchfind ScanResultCard lands in a follow-up. For now, on successful
 * scan the page navigates to /scan?bf_result=<scanId> so the existing
 * ToolScanCard renders the result. That route will get its own redesign next.
 */
  ...
  const submitPhoto = useCallback(async (file) => {
    ...
    try {
      ...
      const resp = await fetch(`${API_URL}/toolscan`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ images: [{ data: base64, media_type: mediaType }] }),
      });
      ...
      // Stash the result so the receiving page can render it without a re-scan.
      // Until the full Benchfind ScanResultCard lands, hand off to the existing
      // /scan route which renders ToolScanCard under the new chrome.
      try {
        sessionStorage.setItem(
          'benchfind_pending_result',
          JSON.stringify({ scanId: data.scanId, imagePaths: data.imagePaths || [], results: data.results }),
        );
      } catch (e) { /* sessionStorage might be unavailable */ }
      navigate(`/scan?from=bf&scanId=${encodeURIComponent(data.scanId || '')}`);
    } catch (err) {
