#!/usr/bin/env node
/**
 * Local CLI runner for the Woodnet ingestion pipeline.
 *
 ...
 *
 * Usage:
 *   node functions/ingest/run-woodnet.js                             # full scrape
 *   node functions/ingest/run-woodnet.js --max-pages 2               # first 2 pages
 *   node functions/ingest/run-woodnet.js --max-threads 20            # first 20 new threads
 *   node functions/ingest/run-woodnet.js --dry-run --max-pages 1 --max-threads 5
 *
 * Requirements:
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (a === '--dry-run') args.dryRun = true;
      ...
      console.log('Usage: node functions/ingest/run-woodnet.js [--max-pages N] [--max-threads N] [--dry-run]');
      process.exit(0);
    }
  }
...
function initAdmin() {
  ...
  try {
    ...
    console.log(`[run-woodnet] using service account at ${saPath}`);
  } catch (e) {
    admin.initializeApp();
    console.log('[run-woodnet] using default application credentials');
  }
}
...
async function main() {
  ...
  const wn = require('./woodnet');

  if (args.dryRun) {
    const { scraped, newRecords } = await wn.scrapeAll({
      maxPages: args.maxPages,
      maxNewThreads: args.maxThreads,
      skipFirestoreLookup: true,
    });

    console.log(`[run-woodnet] dry run — scraped ${scraped} threads from list pages.`);
    console.log(`[run-woodnet] fetched detail for ${newRecords.length} threads (skipped ${scraped - newRecords.length} via max-threads cap or errors).`);
    ...
    console.log('[run-woodnet] sample:', JSON.stringify(sample, null, 2));
    ...
    console.log('[run-woodnet] heuristic brand distribution:', brandCounts);
    console.log('[run-woodnet] heuristic type distribution:', typeCounts);
    console.log(`[run-woodnet] null prices: ${nullPrice}, zero-image: ${zeroImages}, null description: ${nullDesc}`);
    return;
  }

  const summary = await wn.runIngestion({ maxPages: args.maxPages, maxNewThreads: args.maxThreads });
  console.log('[run-woodnet] done:');
...
main()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('[run-woodnet] failed:', err);
    process.exit(1);
  });
